...
class LightllmClientPredictor(BasePredictor):
    """
    A lightweight client that talks to a LightLLM server via OpenAI-compatible endpoints.

    Expected server endpoints:
      - GET  {base_url}/v1/models
      - POST {base_url}/v1/chat/completions

    For multimodal input, we send messages content with text + image_url (data URI, base64-encoded).
    """
    ...
    def __init__(
        ...
        self.model_name = self.get_model_name(base_url)
        self.server_url = f"{base_url}/generate"
        self.stream_url = f"{base_url}/generate_stream"
    ...
    def get_model_name(self, base_url: str) -> str:
        try:
            response = httpx.get(f"{base_url}/get_model_name", timeout=self.http_timeout)
        except httpx.ConnectError:
            raise RuntimeError(f"Failed to connect to server {base_url}. Please check if the server is running.")
        if response.status_code != 200:
            raise RuntimeError(
    ...
    def build_request_body(
        ...
        images = [{'type': "base64", "data": b64encode(image).decode("utf-8")}]
        return {
            "inputs": prompt,
    ...
    def stream_predict(
        ...
        response = requests.post(self.stream_url, json=request_body, stream=True)
        if response.status_code != 200:
            print(response.json())
